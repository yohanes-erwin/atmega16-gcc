...
/**
  ...
  * @param	None
  * @retval	Connection result to DS1307 chip 
  *			(DS1307_CONNECTED or DS1307_NOT_CONNECTED)
  ******************************************************************************
  */
uint8_t ds1307_init()
{
	...
	// Check if DS1307 chip is not properly connected
	if (i2c_is_device_connected(DS1307_DEVICE_ADDRESS) == ERROR)
		return DS1307_NOT_CONNECTED;
	
	return DS1307_CONNECTED;
}
...
/**
  ******************************************************************************
  *	@brief	Write data to DS1307 RAM
  * @param	RAM address (0x08 - 0x3F)
  * @param	RAM value
  * @retval	None
  ******************************************************************************
  */
void ds1307_write_ram(uint8_t address, uint8_t val)
{
	i2c_write_with_reg(DS1307_DEVICE_ADDRESS, 
		check_min_max(address, DS1307_MINIMUN_RAM_ADDRESS,
		DS1307_MAXIMUM_RAM_ADDRESS), val);
}

/**
  ******************************************************************************
  *	@brief	Read DS1307 RAM value
  * @param	RAM address (0x08 - 0x3F)
  * @retval	RAM value
  ******************************************************************************
  */
uint8_t ds1307_read_ram(uint8_t address)
{
	uint8_t ram;
	i2c_read_with_reg(DS1307_DEVICE_ADDRESS, 
		check_min_max(address, DS1307_MINIMUN_RAM_ADDRESS, 
		DS1307_MAXIMUM_RAM_ADDRESS), &ram);
